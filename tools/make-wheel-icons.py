...
def make_ring(size=400, inner=0.609, outer=0.97):
    """The wheel's backing ring.

    Team Beef's is a stippled black annulus - a checkerboard, not a solid fill,
    so it reads as a ring without hiding what is behind it. Measured off theirs:
    the band runs from 0.609 to 0.97 of the half-width and takes every pixel
    where (x + y) is even. Generated that way this matches their own image on
    98.4% of pixels, which is as close as a formula gets to hand-drawn art.
    """
    ...
    for y in range(size):
        for x in range(size):
            radius = math.hypot(x - centre, y - centre) / centre

            if inner <= radius <= outer and (x + y) % 2 == 0:
                put[x, y] = (0, 0, 0, 255)

    return image


def make_cursor(size=10):
    """The wheel's cursor: a small X, black with a grey edge, as theirs is."""
    image = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    put = image.load()

    for y in range(size):
        for x in range(size):
            # Distance to whichever diagonal is nearer, in pixels.
            d = min(abs(x - y), abs(x + y - (size - 1)))
...
def icons_wanted(source_file):
    ...
    Read out of the wheel tables in the engine source, so the two cannot drift
    apart - and cached beside this script, because a release has the tool but
    not the source. Running this from the repository refreshes the cache; a
    downloaded release reads it.
    """
    if not os.path.isfile(source_file):
        if not os.path.isfile(MANIFEST):
            raise SystemExit("no %s and no %s to fall back on"
                             % (source_file, MANIFEST))

        wanted = {}
        for line in open(MANIFEST, encoding="latin-1"):
            line = line.strip()
            if not line or line.startswith("#"):
                continue
            pack, name = line.split(":", 1)
            wanted.setdefault(pack, []).append(name)
        return wanted
    ...
    with open(MANIFEST, "w", newline="\n") as handle:
        handle.write("# Which wheel icons each game needs, read out of the tables\n")
        handle.write("# in src/client/cl_screen.c. Regenerated by running this\n")
        handle.write("# script from the repository; used as-is in a release.\n")
        for pack in sorted(wanted):
            for name in wanted[pack]:
                handle.write("%s:%s\n" % (pack, name))
...
def main():
    ...
    # Anything already in baseq2/wheel is used as it is - Team Beef's art when
    # the owner has their standalone, or a previous run's. The mission packs
    # resolve to it through the search path, so only what is missing is drawn.
    have = set()
    ...
    for pack, names in wanted.items():
        # baseq2 is first, and once it has been done its wheel is what the
        # packs resolve to, so re-read it here rather than once at the start.
        if os.path.isdir(tb_wheel):
            have = {f[:-4] for f in os.listdir(tb_wheel) if f.endswith(".png")}
        ...
        # they are drawn. Only when they are absent: Team Beef's own are better
        # and this never overwrites them.
        if pack == "baseq2":
            ring_dir = os.path.join(install_dir, "baseq2", "wheel")
            os.makedirs(ring_dir, exist_ok=True)
            for name, draw in (("ring", make_ring), ("cursor", make_cursor)):
                target = os.path.join(ring_dir, name + ".png")
        ...
        written, missing = [], []
        for name in names:
            if name in have and not (pack == "baseq2" and
                    not os.path.isfile(os.path.join(out_dir, name + ".png"))):
                continue    # already drawn, or Team Beef's own art covers it

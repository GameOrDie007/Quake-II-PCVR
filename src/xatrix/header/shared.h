 ...
 * =======================================================================
 *
 * This file is the engine's src/common/header/shared.h, copied verbatim with
 * only the include guard renamed.
 *
 * It is a copy rather than an edit of yquake2's own because Team Beef changed
 * this header in ways that move every field the game library and the engine
 * pass between them: pmove_state_t's origin, velocity, gravity and
 * delta_angles all became floats, usercmd_t's movement did too, and
 * player_state_t gained weapmodel. Measured with offsetof, their
 * player_state_t is 208 bytes where stock's is 184, and pmove_state_t 48 where
 * stock's is 28. A mission pack built against its own copy would read every
 * one of those fields at the wrong offset.
 *
 * Keeping a copy means the layouts cannot drift apart later. If this file and
 * the engine's ever differ by more than the guard, that is a bug.
 *
 * =======================================================================
 */
